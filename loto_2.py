...
def generate_numbers(count, min_bound, max_bound):
    if count > max_bound - min_bound + 1:
        raise ValueError('Incorrect input parameters')
    ret = []
    while len(ret) < count:
        i = random.randint(min_bound, max_bound)
        if i not in ret:
            ret.append(i)
    return ret
...
class Loto:
    ...
    def __init__(self):
        ...
        self.__kegs = generate_numbers(self.__num_kegs, 1, 90)
    ...
    def play_round(self):
        ...
        if user_answer == 'y' and not keg in self.__user_card or user_answer != 'y' and keg in self.__user_card and \
                user_answer != 'y' or user_answer != 'n':
            return 2
